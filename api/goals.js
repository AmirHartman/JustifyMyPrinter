...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const rows = await sql`INSERT INTO goals (id, name, target_amount, sort_order, public_visible, public_label)
        VALUES (${randomUUID()}, ${String(body.name).trim()}, ${target}, ${Number(body.sortOrder) || 0}, ${Boolean(body.publicVisible)}, ${String(body.publicLabel || '').trim()}) RETURNING *`;
      return res.status(201).json(rows[0]);
    }
    if (req.method === 'PUT' && id) {
      const body = await parseBody(req); const saved = body.saved === undefined ? null : Number(body.saved);
      if (saved !== null && (!Number.isFinite(saved) || saved < 0)) return res.status(400).json({ error: 'Saved must be non-negative' });
      ...
      if (saved !== null && saved > Number(funds[0].available)) return res.status(400).json({ error: 'ההקצאה חורגת מהיתרה הפנויה בקופת העסק' });
      const rows = await sql`UPDATE goals SET
        saved = CASE WHEN ${saved}::numeric IS NULL THEN saved ELSE LEAST(${saved}, target_amount) END,
        achieved_at = CASE WHEN ${saved}::numeric IS NULL THEN achieved_at WHEN ${saved} >= target_amount THEN COALESCE(achieved_at, NOW()) ELSE NULL END,
